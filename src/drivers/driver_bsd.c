...
bsd_ctrl_iface(void *priv, int enable)
{
	...
	if (ioctl(drv->global->sock, SIOCSIFFLAGS, &ifr, sizeof(ifr)) < 0) {
...
#ifdef __HAIKU__
	haiku_register_events(NULL, global, "global", &global->events,
			wpa_driver_haiku_event);
...
#else
	haiku_unregister_events(global->events);
#endif
